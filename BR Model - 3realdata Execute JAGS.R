...
if(simrun != "Y"){
  ...
  for(i in 5:nrow(n.surv.A.init)){
    ...
    for(j in 2:ncol(n.surv.A.init)){
      ...
      for(j in 2:ncol(totharv.A)){
        if(N.A.init[i,j] < n.surv.A.init[i,j-1] + n.surv.J.init[i,j-1]) {
        x <- N.A.init[i,j] - n.surv.A.init[i,j-1] - n.surv.J.init[i,j-1]
        n.surv.A.init[i,j-1] <- n.surv.A.init[i,j-1] + ceiling(x/2)
        n.surv.J.init[i,j-1] <- n.surv.J.init[i,j-1] + ceiling(x/2)
        }
      }
  ...
  alpha.r.init <- matrix(0.6931472, ncol = ncol(totharv.A)-1, nrow = nrow(N.A.init))
  alpha.r.init[1:4,] <- NA
  ...
  alpha.r.init <- matrix(0.6931472, ncol = ncol(totharv.A)-1, nrow = C*D)
...
#Model for JAGS
br_w_as_model <- source(file = "BR Model - 2m JAGS Model Code - No Pooling of R.R")$value
