...
N.bias.A.prop <- (as.data.frame(real.N.A) - as.data.frame(N.A.est)[,-1])/as.data.frame(real.N.A)
N.bias.A.prop.byyear <- colMeans(N.bias.A.prop)

N.bias.J.diff <- (as.data.frame(real.N.J) - as.data.frame(N.J.est)[,-1])
N.bias.J.prop <- (as.data.frame(real.N.J) - as.data.frame(N.J.est)[,-1])/as.data.frame(real.N.A)
N.bias.J.prop.byyear <- colMeans(N.bias.J.prop)

#Estimate Bias between realized and estimated R
R.bias <- colMeans(r.matrix[,-1] - R.est[,-1])

sink(paste("BiasResults - SingleRun - 3J Trial ",looprun,".csv", sep = ""))
...
cat('\n')
cat('\n')
cat("Average Proportional Bias by Year - N.A")
cat('\n')
write.csv(N.bias.A.prop.byyear)
cat('\n')
cat('\n')
cat("Average Proportional Bias by Year - N.J")
cat('\n')
write.csv(N.bias.J.prop.byyear)
cat('\n')
cat('\n')
cat("Real N.A")
cat('\n')
