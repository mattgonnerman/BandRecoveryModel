...
function(){##############################################################################################
  ...
  for(i in 1:nvisit){
    eta[i] <- intercept_m +
      ...
      beta_S2W_m*wsr_time[i] + beta_wmd_m[wsr_wmd[i]]
  ...
  for(j in 1:nind){
    for(t in f[j]:n.occasions){
      #Survival Model
      cloglog(m.br[j,t]) <- intercept_m +
        beta_A_m*br_age_s[j,t] + beta_S2W_m*br_s2w[j,t] + beta_wmd_m[br_wmd[j]]
      ...
      s[j,t] <- ifelse(t == f[j], exp(-(m.br[j,t]*weeks2harv[j])),
                       ifelse(
                         t == 1 || t == 3 || t == 5, exp(-(11*m.br[j,t])), exp(-(36*m.br[j,t]))
                              )
                       )
  ...
  #Survival Estiamtes
  S_M_J_S2W <- exp(-(36 * mean(WSR_M_J_S2W[sampledwmd])))
  S_M_A_S2W <- exp(-(36 * mean(WSR_M_A_S2W[sampledwmd])))
  S_M_J_W2S <- exp(-(11 * mean(WSR_M_J_W2S[sampledwmd])))
  S_M_A_W2S <- exp(-(11 * mean(WSR_M_A_W2S[sampledwmd])))
