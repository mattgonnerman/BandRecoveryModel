...
WSR.all <- read.csv(paste("E:/Maine Drive/Analysis/Band Recovery/FinalSim/", trialname, " - Master WSR Bias.csv", sep = "")) %>%
  filter(Name == "Bias") %>%
  group_by(Age, Trial) %>%
  summarise(Mean = mean(Estimate))
HR.all <- read.csv(paste("E:/Maine Drive/Analysis/Band Recovery/FinalSim/", trialname, " - Master HR Bias.csv", sep = "")) %>%
  filter(Model %in% c("K")) %>%
  filter(Name == "Bias") %>%
  group_by(Age, Trial) %>%
  summarise(Mean = mean(Estimate))
N.all <- read.csv(paste("E:/Maine Drive/Analysis/Band Recovery/FinalSim/", trialname, " - Master N Bias.csv", sep = ""), check.names=FALSE) %>%
  filter(Model %in% c("K")) %>%
